...
rows = conn.execute('''
    SELECT tw.thread_id, tw.worktree_name, tw.worktree_directory, tw.project_directory
    FROM thread_worktrees tw
    WHERE tw.status = 'ready' AND tw.worktree_directory IS NOT NULL
''').fetchall()
for r in rows:
...
while IFS= read -r line; do
    WT_DIR=$(echo "$line" | python3 -c "import sys,json; print(json.load(sys.stdin)['worktree_directory'])")
    WT_NAME=$(echo "$line" | python3 -c "import sys,json; print(json.load(sys.stdin)['worktree_name'])")
